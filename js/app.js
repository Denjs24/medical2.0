...
let slider_scientific = new Swiper('.scientific__slider', {
	loop:true,
    pagination: {
      el: ".scientific__dots",
      clickable: true,
...
window.onload = function(){
    ...
    function documentActions(e) {
        ...
        if (targetElement.classList.contains("speakers__btn")){
            e.preventDefault();
            showMore();
            let btnContainer = document.querySelector('.speakers__btn-container');
            btnContainer.style.display = 'none';
        }
    }

    let speakersBox = document.querySelector('.body-speakers');
    let speakersItems = speakersBox.querySelectorAll('.body-speakers__item');
    if (speakersItems) {
        for (let index = 0; index < speakersItems.length; index++) {
            const speakersItem = speakersItems[index];
            speakersItem.style.display = 'none';
        }
        let windowWidth = document.body.offsetWidth;
        if (windowWidth >= 768) {
            for (let index = 0; index < 8; index++) {
                const speakersItemes = speakersItems[index];
                speakersItemes.style.display = 'flex';
            }
        }
        if (windowWidth < 768) {
    ...
    function showMore() {
        ...
        for (let index = 0; index < speakersItems.length; index++) {
            const speakersItemeses = speakersItems[index];
            
            speakersItemeses.style.display = 'flex';
        }
    }
...
if (iconMenu != null) {
	...
	let iconMenuBody = document.querySelector('.mobile-menu');
	iconMenu.addEventListener("click", function (e) {
		if (unlock) {
			body_lock(delay);
			iconMenu.classList.toggle("_active");
			menuBody.classList.toggle("_active");
			iconMenuBody.classList.toggle("_active");
		}
	});
};
function menu_close() {
	...
	iconMenuBody.classList.remove("_active");
}
//=================
//BodyLock
...
if (goto_links) {
	for (let index = 0; index < goto_links.length; index++) {
		...
		goto_link.addEventListener('click', function (e) {
			...
			let menuActives = document.querySelector('.icon-menu')
			if (width>=992) {
				_goto(target_block, 600, 82);
			}
			if(width < 992 && !menuActives.classList.contains('_active')){
				_goto(target_block, 600, 47);
			}
			if (width < 992 && menuActives.classList.contains('_active')) {
				_goto(target_block, 600, 47);
				menu_close();
